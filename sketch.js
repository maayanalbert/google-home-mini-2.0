...
function setup(){
 ...
 //    phraseLight.parseSpeech();
    answerLight = new AnswerLight1(255);
    answerLight.makeDots();
...
function AnswerLight1(opacity){
    ...
    this.makeDots = function(){
        verDistApart = this.distApart * sqrt(3)/2
        xstart = width/2 - (this.numCols * this.distApart)/2
        ystart = height/2 - (this.numRows*verDistApart)/2 
        for(rowInd = 0; rowInd < this.numRows; rowInd ++){
            tempArr = [];
            for(colInd = 0; colInd < this.numCols; colInd ++){
                if(rowInd %2 == 0){
                    xpos = xstart+colInd*this.distApart + .5*this.distApart;
                }else{
                    xpos = xstart+colInd*this.distApart;
                }
                ypos = ystart+ (rowInd+.5)*verDistApart;
...
function dot(xpos, ypos){
    this.xpos = xpos;
    this.ypos = ypos;
    this.distFromCenter = sqrt(pow((width/2-xpos),2) + pow((height/2 -ypos),2))
    this.size = 50 -this.distFromCenter/5;
    this.draw = function(){
        fill(255);
        noStroke();
        if(this.distFromCenter < miniSize/2 - 15){
